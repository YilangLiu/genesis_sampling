...
class PS_Genesis(ABC):
    def __init__(self, env, env_cfg, planner_cfg):
        ...
        self.num_dof = self.rollout_envs._num_actions
        self.storage = None
        self.transition = RolloutStorage.Transition()
        self.init_storage(
            self.num_planner_envs,
            self.T,
            [self.rollout_envs._num_states],
            [None],
            [self.rollout_envs._num_actions]
        )
    ...
    def update(self, shift_time, priv_obs, iterations = 2):
        """
        ...
        for _ in range(iterations):
            ...
            self.rollout_envs.reset_with_states(
    ...
    def rollout(self, actions):
        ...
        for i in range(self.T):
            self.rollout_envs.step(actions[:, i, :])
            obs = self.rollout_envs.get_states()
            ...
            self.transition.rewards = self.rollout_envs.compute_rewards(obs, actions[:, i, :])
